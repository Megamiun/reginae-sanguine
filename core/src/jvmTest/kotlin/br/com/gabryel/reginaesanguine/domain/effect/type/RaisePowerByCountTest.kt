...
class RaisePowerByCountTest {
    ...
    fun `given effect scope doesn't match, when fetching raise amount, should not give any enhancement`() {
        val effect = RaisePowerByCount(amount = 1, scope = ALLIES, status = ENHANCED, target = ANY)
    ...
    @Test
    fun `given effect status and scope match, when fetching raise amount, should give enhancement equal to number of matched cells times amount`() {
        val effect = RaisePowerByCount(amount = 2, scope = ALLIES, status = ENHANCED, target = ANY)
        ...
        enhancement shouldBe 4
    }

    @Test
    fun `given card target doesn't match, when fetching raise amount, should return 0`() {
        val effect = RaisePowerByCount(amount = 1, scope = ALLIES, status = ENHANCED, target = ALLIES)
