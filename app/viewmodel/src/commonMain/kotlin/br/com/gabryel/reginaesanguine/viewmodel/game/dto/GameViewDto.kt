...
data class GameViewDto(
    ...
    fun toState(): State = when (state) {
        is StateDto.Ongoing -> State.Ongoing
        is StateDto.Ended.Tie -> State.Ended.Tie
        is StateDto.Ended.Won -> State.Ended.Won(state.player)
    }

    fun getWinner(): PlayerPosition? = when (val s = state) {
        is StateDto.Ended.Won -> s.player
        else -> null
    }
